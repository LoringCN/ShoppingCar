package cn.huace.common.utils.excel;
            ...
            switch (cell.getCellType()) {
                ...
                case Cell.CELL_TYPE_NUMERIC:
                    if(DateUtil.isCellDateFormatted(cell)){//日期类型
                        ...
                        result = DateFormatUtils.format(dateValue,"yyyy-MM-dd HH:mm:ss");
                    }else{
                        //防止读取的数据自动转成小数，即：1 --> 1.0
...
//                        result = cell.getNumericCellValue();
                ...
                case Cell.CELL_TYPE_FORMULA:
                    ...
                    result = evaluateFormulaCell(cell,formulaValue);
//                    result = cell.getCellFormula();
     ...
     * @return
     */
    public static String evaluateFormulaCell(Cell cell,CellValue cellValue){
        Object result = "";
        if (cell != null) {
     ...
     * @param response
     */
    public void exportExcel(List<String> header, List<Map<String,String>> content, String fileName, HttpServletResponse response){
        ...
        for(int i = 0;i< content.size();i++){
            ...
            //创建所有列
            Map<String,String> rowData = content.get(i);
            for(int j = 0;j < headerSize;j++){
                //创建列
                HSSFCell cell = row.createCell(j);
                String cellValue = rowData.get("var"+j);
                //设置样式
        ...
        if(tmpPath != null ){//对临时目录进行处理，防止不同平台出错
            if(tmpPath.contains("/") && !tmpPath.endsWith("/")){
                tmpPath += "/";
            }
            if(tmpPath.contains("\\") && !tmpPath.endsWith("\\")){
                tmpPath += "\\";
        ...
        out("****** 临时文件,file="+ tmpPath + fileName);
        //创建临时文件
...
//        String src_xlspath = "D:\\IDEA\\excel测试数据.xlsx";
...
//            e.printStackTrace();
//        }
//        System.out.println("开始时间 | 截止时间 | 账号名称 | 计划名称 | 展现数 | 公式生成值");
//        for(int i=1;i<rowList.size();i++){
//            Row obj=rowList.get(i);
//            int cellNum = obj.getLastCellNum();
//            for(int j = 0;j<cellNum;j++){
//                System.out.print(getCellValue(obj.getCell(j)));
//            }
//            System.out.println();
//            //System.out.println(""+getCellValue(obj.getCell(0))+"|"+getCellValue(obj.getCell(1))+"|"+getCellValue(obj.getCell(2))+"|"+getCellValue(obj.getCell(3))+"|"+getCellValue(obj.getCell(4))+"|"+getCellValue(obj.getCell(5)));
//        }
        String tmpPath = System.getProperty("java.io.tmpdir");
        if(tmpPath != null ){//对临时目录进行处理，防止不同平台出错
            if(tmpPath.contains("/") && !tmpPath.endsWith("/")){
